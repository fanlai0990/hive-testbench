...
public class AttMpls {
    ...
    static void helper(String input) {
        ...
        Path flog;
        if (input.contains("TPC-DS")) {
            flog = Paths.get("./log_tpcds_2_shuffle_largest_h");

        } else if (input.contains("TPC-H")) {
            flog = Paths.get("./log_tpch_2_shuffle_largest_h");

        } else if (input.contains("BigBench")) {
            flog = Paths.get("./log_bigb_2_shuffle_largest_h");

        } else { // facebook data not generated by hive, need not to be compared
            System.out.println();
            return;
        }

        Map<Integer, Integer> largestM = new HashMap<>();
        try (BufferedReader reader = Files.newBufferedReader(flog, charset)) {
            String line = null;
            while ((line = reader.readLine()) != null) {

                if (line.contains("query")) {
                    String query = line.substring(line.lastIndexOf("query") + "query".length(), line.indexOf("."));
